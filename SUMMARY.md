...
### 手校（`data/moyu_points.json`）

界面底部列表：增删改查、排序、锁定后点镜像记坐标（含 cg/outer 锚点）。

### 脚本步骤（写回 `configs/*.json`）

左侧脚本点 **编辑**：

- 改名称 / 说明 / 是否走通用前置 / 搜索词  
- 步骤 **↑↓ 增删改**；常用类型下拉新增  
- 表单绑 **手校点位**（`calib_key`）+ 关键词 / 模板 / 次数  
- **高级 JSON** 可改任意字段  
- **新建** 空白脚本  

手校与步骤解耦：步骤只存 `calib_key` 引用，坐标在手校表。

### 分组（游戏文件夹）

- **新建分组** → 写入 `configs/catalog.json` + `configs/<id>/`
- 分组行 **+脚本 / 改名 / 删组（空组）**
- **新建脚本** 可选所属分组；编辑里可改「所属分组」（文件会搬家）
- 「崩铁」等别名会落到已有「星穹铁道」分组

### 权限（永不可破）

- **默认允许业务截屏**（OCR/识图）；失败会硬停，避免连环弹窗。
- 打不开识图确认时：仍继续跑脚本步骤（手校点击不依赖 OCR）。
- 窗口偏小会自动 ⌘+ 放大，放大后继续开游戏/跑步骤。
- 界面「重启界面」：只重启 GUI，不重编工具。
